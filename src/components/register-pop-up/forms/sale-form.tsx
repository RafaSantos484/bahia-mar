...
import { CollaboratorType, ProductSale, Sale } from "../../../types";
...
function productsToDict(
  products?: { [id: string]: ProductSale },
  globalState?: GlobalState | null
): {
  [id: string]: { price: number; quantity: number };
} {
  if (!products || !globalState) return {};

  const productsDict: any = {};
...
export default function SaleForm({
  ...
  const [data, setData] = useState({
    collaboratorId: isAdmin
      ? editingData?.collaboratorId || ""
      : globalState?.loggedUser.id || "",
    vehicleId: editingData?.vehicleId || "",
    paymentMethodId: editingData?.paymentMethodId || "",
    client: editingData?.client || "",
    products: productsToDict(editingData?.products, globalState),
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div
        className={isAdmin ? "three-fields-container" : "two-fields-container"}
      >
        {isAdmin && (
          <FormControl required>
            <InputLabel id="collaborator-select-label">Funcionário</InputLabel>
            <Select
